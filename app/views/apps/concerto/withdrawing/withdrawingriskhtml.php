...
<div>
    <div class="uk-container">
        <h1 class="atr-harvesting"><?php echo $this->headerLabel; ?></h1>
        <div class="atr-primary-nav">
            <?php echo doreyjtcSteps('risk'); ?>
            <ul class="uk-child-width-expand uk-margin-small-top" uk-tab>
                <li class="uk-active"
                    onClick="window.location='<?php echo website_url($this->controller.'/withdrawing-risk');?>'"><span
                ...
                <li class="uk-visible@m"
                    onClick="window.location='<?php echo website_url($this->controller.'/withdrawing-results');?>'">
                    <span class="uk-position-center-left uk-hidden@s atr-progress" style="padding-left: 20px;"><span
                            class="complete"></span> <span class="complete"></span> <span class="complete"></span> <span
                            class="complete"></span> <span class="complete"></span> <span
                            class="complete"></span></span><a>Projection</a></li>
            </ul>
        </div>
        ...
        <?php echo form_open(base_url(), 'class="SaveFormInfo" method="POST" data-action="saveWithdrawingtransparency" data-controller="'.$this->controller.'"');?>
        <div class="RiskMatrixRiskTableDataDivisionBox">
            <table id="risk-matrix"
                class="RiskMatrixRiskTableData uk-table uk-table-small atr-table  atr-table column-highlight-<?php echo (!empty($info->risklabel) ? $info->risklabel :'four'); ?>">
            </table>
        </div>
        <?php form_close(); ?>
    </div>
